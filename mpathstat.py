...
#
# Version 1.2
#     linted the program with yapf to fix formatting issues
# Version 1.1
#     Original release
#
        ...
        for line in subproc.stdout.readlines():
            if "sd" in line:
                ...
                if blockdev in blockdevices:
                    blockdevices[blockdev]['await'] = float(line.split()[9].replace(",", "."))
                    blockdevices[blockdev]['reads'] = float(line.split()[3].replace(",", "."))
                    blockdevices[blockdev]['writes'] = float(line.split()[4].replace(",", "."))
                    blockdevices[blockdev]['bytes_read'] = float(line.split()[5].replace(",", "."))
                    blockdevices[blockdev]['bytes_written'] = float(line.split()[6].replace(",", "."))
            elif "dm" in line:
                mapperdev = line.split()[0]
                if mapperdev in mapperdevices:
                    mapperdevices[mapperdev]['await'] = float(line.split()[9].replace(",", "."))
                    mapperdevices[mapperdev]['reads'] = float(line.split()[3].replace(",", "."))
                    mapperdevices[mapperdev]['writes'] = float(line.split()[4].replace(",", "."))
                    mapperdevices[mapperdev]['bytes_read'] = float(line.split()[5].replace(",", "."))
                    mapperdevices[mapperdev]['bytes_written'] = float(line.split()[6].replace(",", "."))
